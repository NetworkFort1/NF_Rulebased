...
const DNSPage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response = await fetch(`${process.env.REACT_APP_BACKEND_BASE_URL}/DNS/getAllDnsLogs`);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response = await fetch(`${process.env.REACT_APP_BACKEND_BASE_URL}/DNS/getAllDNS`);
        const data = await response.json();
        setLogs(data.logs);
      } catch (error) {
        console.error('Error fetching data:', error);
  ...
  const generateChartData = () => {
    ...
    const numberOfRequestsBySourceIPData = {
      ...
      datasets: [
        {
          label: "Number of DNS Requests by Source IP",
          backgroundColor: "rgba(127, 127, 127, 1.0)",
          borderColor: "rgba(127, 127, 127, 1.0)",
          borderWidth: 1,
