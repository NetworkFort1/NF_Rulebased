...
const MitrePage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // Fetch Mitre logs from your backend API
        const response = await fetch(`${process.env.REACT_APP_BACKEND_BASE_URL}/Mitre/getAllMitre`);
  ...
  const processDataForLineGraph = (logs) => {
    ...
    const data = logs.map((log) => (log.attack === 'Mitre_Attacks' ? 1 : 0));

    return {
      labels,
      datasets: [
        {
          label: "Mitre Attacks Over Time",
          fill: false,
          lineTension: 0.1,
  ...
  const generateChartData = () => {
    ...
    const numberOfRequestsBySourceIPData = {
      ...
      datasets: [
        {
          label: "Number of Mitre Requests by Source IP",
          backgroundColor: "rgba(127, 127, 127, 1.0)",
          borderColor: "rgba(127, 127, 127, 1.0)",
          borderWidth: 1,
  ...
  return (
    ...
    <div className="MainDiv">
      <h1>Mitre Inside Hosts</h1>

      <div className="secondBoxDiv">
        <div className="row1">
          <div className="NumberOfRequestsBySourceIP">
            <h4>Number of Mitre Requests by Source IP</h4>
